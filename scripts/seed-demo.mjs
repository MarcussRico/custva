...
const adminPassword =
  process.env.ADMIN_PASSWORD || process.env.CUSTVA_PLATFORM_ADMIN_PASSWORD;

if (!adminPassword) {
  throw new Error(
    "Set ADMIN_PASSWORD or CUSTVA_PLATFORM_ADMIN_PASSWORD before seeding. Do not rely on defaults in shared environments."
  );
}

if (adminPassword === "Admin@123" && process.env.NODE_ENV === "production") {
  throw new Error("Refusing to seed default Admin@123 when NODE_ENV=production");
}
...
console.log(
  "Lifecycle templates are seeded via migration 0010. Run: node scripts/seed-lifecycle-templates.mjs"
);
