/** Auth headers for merchant BFF → API. Dev header only when not production. */
export function merchantAuthHeaders(accessToken?: string): Record<string, string> {
  if (accessToken) {
  ...
  if (!isProd && devMerchantId) {
    return { "x-dev-merchant-id": devMerchantId };
  }
